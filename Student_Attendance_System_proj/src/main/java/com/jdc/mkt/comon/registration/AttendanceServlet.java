...
public class AttendanceServlet extends HttpServlet {
	...
	private void addToAttendanceList(HttpServletRequest req, HttpServletResponse resp) {
		...
		for (Student a : stuList) {
			int id = a.getId();
			int stu_id = Integer.parseInt(req.getParameter("student_"+id));
			boolean present = Boolean.parseBoolean(req.getParameter("present_"+id));
	...
	private LocalDate getLocalDate(HttpServletRequest req) {
		...
		return dd != null && !dd.isEmpty() ? LocalDate.parse(dd) : LocalDate.now();
	}
