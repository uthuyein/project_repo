...
public class StudentServiceImpl implements StudentService {
	...
	public List<Student> getStudents(int id,String name,String room) {
		...
		try (var con = getConnector(); var stmt = con.prepareStatement(sb.toString())) {
			...
			while (rs.next()) {
				...
				st.setName(rs.getString("st.name"));
				st.setPhone(rs.getString("st.phone"));
	...
	public void updateStudent(Student s) {
		String sql ="update student_tbl set phone=? ,email=?, classroom_id=? where id =?";
		try(var con  = getConnector(); var stmt = con.prepareStatement(sql)){
			
			stmt.setString(1, s.getPhone());
			stmt.setString(2, s.getEmail());
			stmt.setInt(3, s.getClassRoom().getId());
			stmt.setInt(4, s.getId());
			
			updateAddress(s.getAddress());
			stmt.executeUpdate();
	...
	private void updateAddress(Address a) {
		String sql ="update address_tbl set street=?,township=?,city=? where id =?";
		try(var con  = getConnector(); var stmt = con.prepareStatement(sql)){
	...
	public void deleteStudent(int id) {
		String sql ="update student_tbl set isActive=false where id =?";
		try(var con  = getConnector(); var stmt = con.prepareStatement(sql)){
